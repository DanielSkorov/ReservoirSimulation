...
class env2p(unittest.TestCase):
  ...
  def test_04(self):
    ...
    env = env2pPT(pr, Tmin=258.15, miniter=1,
                  flashkwargs=dict(runstab=False, useprev=True))
    maxpoints = 124
    res = env.run(P0, T0, yi, 0., maxpoints=maxpoints, maxstep=0.3)
    if plotting:
  ...
  def test_07(self):
    ...
    pr = pr78(Pci, Tci, wi, mwi, vsi, dij)
    env = env2pPT(pr, Tmin=328.15, psatkwargs=dict(tol=1e-20),
                  flashkwargs=dict(runstab=False, useprev=True))
    maxpoints = 99
    res = env.run(P0, T0, yi, 0., maxpoints=maxpoints)
    if plotting:
  ...
  def test_21(self):
    ...
    pr = pr78(Pci, Tci, wi, mwi, vsi, dij)
    env = env2pPT(pr, Tmin=273., Tmax=673., Pmax=150e6,
                  flashkwargs=dict(runstab=False, useprev=True))
    maxpoints = 207
    res = env.run(P0, T0, yi, 0., maxpoints=maxpoints, sidx0=15, maxstep=0.1)
    if plotting:
      self.plot(res, 0., 400., 0., 150.)
    self.assertTrue(res.Pk.shape[0] == maxpoints)
    pass
