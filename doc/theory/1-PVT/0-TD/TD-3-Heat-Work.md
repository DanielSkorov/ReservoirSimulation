...
<a id='pvt-td-heatwork-quasistatic'></a>
```{admonition} NB
:class: note
Использование *квазистационарного подхода* для описания изменения системы применимо только тогда, когда время, необходимое на изменение положения цилиндра (нарушения состояния системы), значительно больше времени ее релаксации.
```

Иными словами, *квазистационарным* называется процесс, при котором изменение объема системы происходит при условии одинаковости давления во всей системе. Такой процесс, как быстрое сжатие (расширение) газа в цилиндре, когда вблизи поршня образуется избыток (недостаток) молекул газа, что приводит к неравномерному распределению давления в системе, нельзя отнести к квазистационарным по определению. Еще одним примером не квазистационарного процесса является процесс свободного расширения газа в вакуум, поскольку он протекает практически мгновенно из-за большой разницы давлений, к тому же является необратимым (подробнее данный пример будет рассмотрен в разделе, посвященном [энтропии](TD-6-Entropy.md).)

```{admonition} Дополнительно
:class: note
Дифференциальная форма записи первого начала термодинамики предполагает использование $\delta$ вместо $d$ перед количеством тепла $Q$ и работой $W$:

$$ dU = \delta Q + \delta W. $$

Это обусловлено тем, что количество тепла $Q$ и работа $W$ не всегда являются [термодинамическими функциями состояния](https://en.wikipedia.org/wiki/State_function), в отличие от, например, внутренней энергии. Функцией состояния называют физическую величину, представляющую собой функцию от термодинамических [параметров](TD-1-Basics.md#pvt-td-basics-observables). Например, функция $\Psi \left( P, \, V \right) = PV$ является функцией состояния, поскольку зависит только от термодинамических параметров. Однако работа $W$ представляет собой некоторое количество энергии, передаваемое системой(е) в процессе совершения работы. Аналогично и для количества тепла $Q$. При этом обе физические величины, работа и тепло, зависят от рассматриваемого процесса. Следовательно, $\delta W$ и $\delta Q$ используются для обозначения бесконечно малых количеств энергии, передаваемых системе (или самой системой) в виде работы и тепла соответственно, иными словами, их приращений.
```
