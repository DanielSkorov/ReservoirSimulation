...
* [Приложение D. О решении уравнений состояния](EOS-Appendix-D-Solve.md)

В [предыдущем разделе](../0-TD/TD-0-Introduction.md) были введены основные термодинамические [параметры](../0-TD/TD-1-Basics.md#pvt-td-basics-observables) системы, характеризующие ее [состояние](../0-TD/TD-1-Basics.md#pvt-td-basics-state). Также были рассмотрены [законы](../0-TD/TD-1-Basics.md#pvt-td-basics-laws), которым подчиняется любая термодинамическая система. Этот раздел будет посвящен изучению ***правил*** (уравнений состояния), устанавливающих взаимосвязи между параметрами системы. Уравнения состояния представляют собой математические выражения, устанавливающие взаимосвязи между параметрами *гомогенной* системы, и полученные на основе экспериментальных данных и наблюдений за поведением реальных систем. То есть уравнения состояния являются эмпирическими зависимостями. Как и любые эмпирические зависимости, уравнения состояния характеризуются *областью применения* – такими диапазонами изменения параметров системы, при которых рассматриваемое уравнение состояния соответствует экспериментальным данным.

```{admonition} Дополнительно
:class: note
В классической термодинамике определение уравнениям состояния дается следующим образом:

```{admonition} Определение
:class: tip
***Уравнения состояния*** позволяют выразить [интенсивные параметры](../0-TD/TD-9-Observables.md) термодинамической системы через независимые [экстенсивные параметры](../0-TD/TD-9-Observables.md). Например,

$$ \begin{cases}
T &= T \left( S, \, V, \, \mathbf{n} \right), \\
P &= P \left( S, \, V, \, \mathbf{n} \right), \\
\mu_i &= \mu_i \left( S, \, V, \, \mathbf{n} \right), \; i = 1 \, \ldots \, N_c,
\end{cases} $$

где $T, \, P, \, \mu_i, \, i = 1 \, \ldots \, N_c,$ – интенсивные параметры (температура, давление и химический потенциал $i$-го компонента соответственно), $S, \, V, \, n_i, \, i = 1 \, \ldots \, N_c,$ – экстенсивные параметры (энтропия, объем и количество вещества $i$-го компонента соответственно).

Таким образом, одно уравнение состояния не предоставляет всю необходимую информации о системе. Однако для большинства задач, рассматриваемых в рамках данного и последующих разделов, наличие одного уравнения состояния является *достаточным*. Кроме того, в практике неизотермического гидродинамического моделирования, когда появляется необходимость определения термодинамических потенциалов (например, энтальпии для решения уравнения энергетического баланса), используются корреляции для расчета их значений при условии нахождения системы в состоянии идеального газа и необходимые *поправки* (*[departure functions](https://en.wikipedia.org/wiki/Departure_function)*), полученные из известного уравнения состояния, позволяющие перейти от рассмотрения идеальных условий к реальным; а также табличные значения термодинамических потенциалов, если речь идет о чистых компонентах.
```
