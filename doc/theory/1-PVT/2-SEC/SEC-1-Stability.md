...
где $N_p$ – количество фаз в системе, а $N_c$ – число компонентов. При известных и заданных давлении и температуре рассматриваемой системы, а также количествах вещества компонентов в ней [энергия Гиббса](../0-TD/TD-10-MixtureGibbsEnergy.md) некоторой фазы определяется количеством вещества каждого компонента в фазе, то есть:

$$ G_j = G_j \left( \mathbf{n}_j \right), \; j = 1 \, \ldots \, N_p. $$

Учитывая [экстенсивность](../0-TD/TD-9-Observables.md#pvt-td-observables-extensive) энергии Гиббса, для многофазной системы справедливо следующее соотношение:
...
В данном определении необходимо отметить два момента. Первый: использование функции энергии Гиббса в качестве термодинамического потенциала для определения *равновесного состояния* всей многофазной системы допустимо при рассмотрении изобарно-изотермического (или [квазистационарного](../TD-0/TD-3-Heat-Work.md)) процесса. Более подробно это рассматривалось [ранее](../0-TD/TD-14-PhaseEquilibrium.md). Второй: для того чтобы доказать, что некоторое состояние $\hat{M}$ многофазной системы из $N_p$ фаз является *равновесным*, мы можем рассмотреть множество других состояний $\hat{K}$, каждое из которых будет характеризоваться своим числом фаз $K_p$, и если в состоянии $\hat{M}$ с количеством фаз $N_p$ энергия Гиббса меньше всего, то оно и будет равновесным.
...
Учитывая [правило нахождения производной сложной функции](https://en.wikipedia.org/wiki/Chain_rule), данное выражение преобразуется следующим образом:
...
Поскольку частная производная энергии Гиббса некоторой фазы по количеству вещества $i$-го компонента в этой фазе при фиксированных давлении и температуре [соответствует](../0-TD/TD-10-MixtureGibbsEnergy.md) химическому потенциалу $i$-го компонента в этой фазе, то:
...
При фиксированных давлении и температуре для энергии Гиббса выполняется следующее [свойство](../0-TD/TD-10-MixtureGibbsEnergy.md):
...
Далее будет показано, что при любых значениях $\mathbf{r}$ в равновесном состоянии функция $D \left( \mathbf{r} \right)$ не принимает отрицательные значения. Однако, прежде чем переходить к доказательству этого утверждения, необходимо рассмотреть следующую лемму:
...
```{admonition} Доказательство
:class: proof
Рассмотрим любое состояние системы $\hat{K} = \left\{ \mathbf{r}_k, \, k = 1 \, \ldots \, K_p \right\}$. Тогда из условия $D_k \left( \mathbf{r}_k \right) \geq 0, \, k = 1 \, \ldots \, K_p,$ следует:
...
```{admonition} Дополнительно
:class: note
При выводе данного выражения было использовано [уравнение Гиббса-Дюгема](../0-TD/TD-12-GibbsDuhemEquation.md).
```
````
...
```{admonition} Пример
:class: exercise
Пусть имеется 1 моль смеси из метана и диоксида углерода при температуре 40 °C и давлении 2 МПа с мольной долей метана 0.85. Необходимо определить, является ли однофазное состояние данной системы стабильным.
```

````{dropdown} Решение
Для решения данной задачи будем использовать [уравнение состояния Пенга-Робинсона](../1-EOS/EOS-2-SRK-PR.md), реализованное [здесь](https://github.com/DanielSkorov/ReservoirSimulation/tree/main/resim/pvt/eos/eos.py).
...
``` python
eos = pr78(Pci, Tci, wi, mwi, vsi, dij)
```

Выполним расчет приведенной энергии Гиббса для всех возможных компонентных составов рассматриваемой смеси. Для расчета коэффициентов летучестей компонентов для различных составов будем использовать метод `getPT_lnphiji`, принимающий в качестве аргументов давление (в Па), температуру (в K) и набор компонентных составов в виде двумерного массива (размерностью `(Np, Nc)`, где `Np` – количество наборов компонентных составов, `Nc` – количество компонентов в каждом компонентном составе), и возвращающий соответствующие коэффициенты летучести компонентов в виде двумерного массива (матрицы) размерности `(Np, Nc)`. Затем вычислим летучести компонентов и энергии Гиббса для соответствующих компонентных составов.

``` python
xj1 = np.linspace(1e-4, 0.9999, 100, endpoint=True)
xji = np.column_stack([xj1, 1. - xj1])
lnphiji = eos.getPT_lnphiji(P, T, xji)
lnfji = lnphiji + np.log(P * xji)
Gj = np.vecdot(xji, lnfji)
```

Функция [`np.linspace`](https://numpy.org/doc/stable/reference/generated/numpy.linspace.html) позволяет создать одномерный массив, заданное количество элементов которого (в рассматриваемом примере – `100`) равномерно (с постоянным шагом) распределено в необходимом интервале (в данном случае – от `1e-4` до `0.9999` включительно). Функция [`np.column_stack`](https://numpy.org/doc/stable/reference/generated/numpy.column_stack.html) позволяет создать двухмерный массив (матрицу) из нескольких одномерных (векторов) путем их объединения и размещения в виде столбцов. В рассматриваемом примере с использованием данной функции создается матрица различных компонентных составов рассматриваемой системы размерностью `(100, 2)` из двух векторов, размерность каждого из которых составляет `(100,)`. Функция [`np.vecdot`](https://numpy.org/doc/stable/reference/generated/numpy.vecdot.html) позволяет выполнить [скалярное произведение векторов](../../0-Math/0-LAB/LAB-2-VectorOperations.md#math-lab-vectoroperations-innerproduct) путем суммирования произведений елементов массивов вдоль последней оси. В данном примере она используется для вычисления элементов одномерного массива $G_j = \sum_{i=1}^{N_c} x_{ji} \ln f_{ji}, \, j = 1 \, \ldots \, N_p$.

Теперь определим летучести компонентов для заданного компонентного состава. Для этого будем использовать метод `getPT_lnfi`, принимающий на вход давление (в Па), температуру (в K) и компонентный состав в виде одномерного масива (размерностью `(Nc,)`) и возвращающий вектор логарифмов летучести компонентов. Затем вычислим значения касательной к функции энергии Гиббса в точке с рассматриваемым компонентным составом.

``` python
lnfi = eos.getPT_lnfi(P, T, yi)
Lj = xji.dot(lnfi)
```
...
```{admonition} Пример
:class: exercise
Пусть имеется 1 моль смеси из метана и диоксида углерода при температуре 10 °C и давлении 6 МПа с мольной долей метана 0.1. Необходимо определить, является ли однофазное состояние данной системы стабильным.
```
...
Выполним расчет приведенной энергии Гиббса для всех возможных компонентных составов рассматриваемой смеси. Затем определим летучести компонентов и энергии Гиббса для соответствующих компонентных составов. Поскольку свойства компонентов не изменились, то для вычисления термодинамических параметров будем использовать инициализированный в предыдущем примере класс с уравнением состояния.
...
```{admonition} Дополнительно
:class: note
Важно отметить, что рассматриваемый подход к проверке стабильности фазового состояния однофазной системы можно использовать и для проверки стабильности многофазной системы: для этого достаточно проверить на стабильность компонентный состав любой из фаз, применив данный метод, если свойства каждой из фаз рассчитываются с использованием одного и того же уравнения состояния.
```
````

```{admonition} NB
:class: note
Критерием стабильности рассматриваемого фазового состояния системы с количествами вещества компонентов $\mathbf{n}$ является:
...
В процессе этого преобразования было использовано [соотношение Гиббса-Дюгема](../0-TD/TD-12-GibbsDuhemEquation.md) при постоянных давлении и температуре:
...
Для квазистационарного изотермического процесса разность химических потенциалов [можно](../0-TD/TD-15-Fugacity.md#pvt-td-fugacity-realgasmixture) заменить разностью логарифмов летучестей компонентов. Следовательно, условие стационарности функции TPD записывается следующим образом:
...
Преобразуем данное выражение с учетом [определения коэффициента летучести](../0-TD/TD-15-Fugacity.md):
...
``` python
_, lnphiyi = eos.getPT_lnphii(P, T, yi)
...
pupdate = partial(update_ss, hi=hi, yi=yi, plnphi=partial(eos.getPT_lnphii, P=P, T=T))
```
...
for i, ki in enumerate(K):
    ni = ki * yi
    xi = ni / ni.sum()
    _, lnphixi = eos.getPT_lnphii(P, T, xi)
...
```{admonition} Дополнительно
:class: note
Используя [уравнение Гиббса-Дюгема](../0-TD/TD-12-GibbsDuhemEquation.md) при постоянных (фиксированных) давлении и температуре, получим:

$$ \sum_{i=1}^{N_c} n_i d \mu_i = 0. $$

С учетом [соотношения](../0-TD/TD-15-Fugacity.md) между дифференциалом химического потенциала $i$-го компонента и дифференциалом логарифма его летучести данное равенство преобразуется следующим образом:
...
Далее воспользуемся соотношением между частными производными логарифма летучести и логарифма коэффициента летучести, полученным [ранее](../1-EOS/Appendix-A-PD.md):
...
При выводе данного выражения использовалось [уравнение Гиббса-Дюгема](../0-TD/TD-12-GibbsDuhemEquation.md).
...
Аналитические выражения частных производных логарифма коэффициента летучести с использованием уравнений состояния были получены [ранее](../1-EOS/Appendix-A-PD.md).
...
pupdate = partial(update_newt, hi=hi, plnphi=partial(eos.getPT_Z_lnphii_dnj, P=P, T=T))
```
