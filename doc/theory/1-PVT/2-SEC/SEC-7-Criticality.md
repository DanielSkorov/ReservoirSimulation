...
Под критической состоянием понимается такое состояние системы, где исчезает различие между двумя фазами. Для однокомпонентных систем такое состояние представляет собой точку, то есть, согласно [правилу фаз Гиббса](../0-TD/TD-11-GibbsPhaseRule.md), необходимо знать два параметра (например, давление и температуру), чтобы определить критическое состояние такой системы. Для двухкомпонентной системы количество степеней свободы будет равно трем, то есть такая система будет характеризоваться линией из критических состояний, каждое из которых соответствует своему компонентному составу, ограниченной с двух концов критическими точками чистых компонентов.
```
````
...
Поскольку определитель матрицы [равен](../../0-Math/0-LAB/LAB-8-Eigenvalues-Eigenvectors.md#characteristic-polynomial) произведению собственных значений, то граница метастабильной области (спинодаль) определяется следующим выражением:
...
Остальные обозначения, в том числе $v, \, \delta_1, \, \delta_2, \, \alpha_{ij}, \, b_i, \, \alpha_m, \, b_m$, соответствуют обозначениям, используемым при рассмотрении [кубических уравнений состояния](../1-EOS/EOS-2-SRK-PR.md).
...
```{code-cell} python
zetai, lmbdk = pyrqi(Q, yi)
print(f'Minimum eigenvalue is: {lmbdk:.3f}')
```
...
```{code-cell} python
tmpl = '%3s%9.2f%8.3f%13.3e'
print('%3s%9s%8s%13s' % ('Nit', 'T, K', 'lmbd', 'ΔT / T'))
print(tmpl % (k, Tk, lmbdk, dT))
...
Представленный алгоритм нашел решение за семь итераций (включая начальную). Полученный вектор собственных значений является единичным:
...
В последующем будем использовать [реализацию](https://github.com/DanielSkorov/ReservoirSimulation/blob/main/_src/envelope.py) данного алгоритма в виде функции `getVT_Tspinodal`:

```{code-cell} python
from envelope import getVT_Tspinodal
```
...
После решения уравнения $\lambda_{min} = 0$ выполняется $j$-я итерация внешнего цикла решения уравнения $C = \delta^3 F = 0$. Данное уравнение при использовании [кубических уравнений состояния](../1-EOS/EOS-2-SRK-PR.md) рекомендуется решать относительно безразмерного параметра $\kappa$:
...
```{code-cell} python
tmpl = '%3s%8.3f%9.2f%13.3e%13.3e'
print('%3s%8s%9s%13s%13s' % ('Nit', 'ϰ', 'T, K', 'C*', 'Δϰ / ϰ'))
print(tmpl % (j, kappaj, T, Cj, dkappa))
