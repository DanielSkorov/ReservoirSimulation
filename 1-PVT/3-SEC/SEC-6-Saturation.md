...
Для расчета логарифмов коэффициентов летучести компонентов будем использовать метод `getPT_lnphii` инициализированного класса с уравнением состояния, принимающий на вход давление (Па), температуру (K) и компонентный состав, и возвращающий вектор логарифмов коэффициентов летучести компонентов. Поскольку необходимо рассчитать значения логарифмов коэффициентов летучести компонентов для различных давлений, воспользуемся функцией [`numpy.vectorize`](https://numpy.org/doc/stable/reference/generated/numpy.vectorize.html), позволяющей вычислить результат некоторой функции (или метода класса) для набора входных данных и обратить итог в `numpy.array`.

```{code-cell} python
plnphiji = np.vectorize(pr.getPT_lnphii, excluded=(1, 2), signature='()->(n)')
```
...
lnphizji = plnphiji(Pj, T, zi)
lnphiyji = plnphiji(Pj, T, yi)
```
