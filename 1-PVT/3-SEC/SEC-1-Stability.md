...
$$ k_i^{go} = \frac{{P_c}_i}{P} \exp \left( 5.3727 \left(1 + \omega_i \right) \left(1 - \frac{{T_c}_i}{T} \right) \right). $$
...
При использовании метода Ньютона для решения задачи оптимизации обновление основных переменных $\alpha_i, \, i = 1 \, \ldots \, N_c,$ на $k$-й итерации осуществляется с использованием следующего выражения:

$$ \mathbf{\alpha}^{k+1} = \mathbf{\alpha}^{k} + \Delta \mathbf{\alpha}^{k}, $$
