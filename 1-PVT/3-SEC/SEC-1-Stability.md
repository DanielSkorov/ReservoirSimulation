...
Пусть $Y_i = y_i \exp \left( -\bar{K} \right)$, тогда условие стационарности функции TPD:
...
Стоит отметить, что значения $Y_i$ можно условно интерпретировать как количество вещества $i$-го компонента, действительно:
...
Тогда систему нелинейных уравнений, определяющих условие стационарности функции TPD, можно преобразовать к следующему виду:
...
Если использовать в качестве основных переменных мольные доли компонентов для произвольного состава $y_i$, то такая задача будет являться *задачей условной минимизации или задачей с ограничениями (constrained minimization)*. В качестве условий будут выступать следующие соотношения:
...
Как уже было отмечено ранее, при использовании $Y_i, \, i = 1 \, \ldots \, N_c,$ в качестве основных переменных положение стационарных точек функции TPD будет определяться следующей системой нелинейных уравнений:

$$ g_i = \ln Y_i + \ln \phi_i \left( \mathbf{y} \right) - h_i = 0, \; i = 1 \, \ldots \, N_c. $$

При этом в работе \[[Michelsen, 1982](https://doi.org/10.1016/0378-3812(82)85001-2)\] автором было предложено использовать вместо функции TPD ее модификацию:

$$ \bar{D}^* \left( \mathbf{Y} \right) = 1 + \sum_{i=1}^{N_c} Y_i \left( \ln Y_i + \ln \phi_i \left( \mathbf{y} \right) - h_i - 1 \right). $$

В рамках задачи поиска локальных минимумов функции TPD и проверки ее значений в них замена функции возможна при условии одинаковости положения (координат) стационарных точек и значений функции в них. Следовательно, необходимо получить систему уравнений, характеризующих положения стационарных точек модифицированной функции TPD, которые определяются равенством нулю частных производных функции по основным переменным:

````{margin}
```{admonition} Дополнительно
:class: note
Используя [уравнение Гиббса-Дюгема](../1-TD/TD-12-GibbsDuhemEquation.md) для квазистационарного изотермического процесса, получим:
...
Таким образом, положения стационарных точек исходной и модифицированной функций TPD определяются одной и той же системой нелинейных уравнений. Получим выражение для модифицированной функции TPD в стационарных точках:
