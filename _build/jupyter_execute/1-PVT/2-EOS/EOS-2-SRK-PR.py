...
class eos_pr_srk(core):
    ...
    def calc_Z_V(self):
        return self.v / (self.v - self.mr.bm) - self.mr.alpham * self.v / (R * self.T *                (self.v**2 + (self.mr.c + 1) * self.mr.bm * self.v - self.mr.c * self.mr.bm**2))
    ...
    def calc_fug_coef_V(self):
        return -np.log((self.v - self.mr.bm) / self.v) + self.mr.bi * (1 / (self.v - self.mr.bm) -                self.mr.alpham * self.v / (R * self.T * self.mr.bm * (self.v + self.mr.bm * self.mr.delta1) *                (self.v + self.mr.bm * self.mr.delta2))) + (2 * np.sum(self.mr.alphaij * self.repeat(self.z, 0), 1) /                self.mr.bm - self.mr.alpham * self.mr.bi / self.mr.bm**2) * np.log((self.v + self.mr.bm * self.mr.delta1) /                (self.v + self.mr.bm * self.mr.delta2)) / (R * self.T * (self.mr.delta2 - self.mr.delta1)) - np.log(self.Z)

    def calc_fug_coef_P(self):
        return -np.log(self.Z - self.B) + self.mr.bi * (self.Z - 1) / self.mr.bm +                self.A * (2 * np.sum(self.mr.alphaij * self.repeat(self.z, 0), 1) / self.mr.alpham - self.mr.bi / self.mr.bm) *                np.log((self.Z + self.B * self.mr.delta1) / (self.Z + self.B * self.mr.delta2)) /                (self.B * (self.mr.delta2 - self.mr.delta1))

    def calc_dG(self, Z1, Z2):
        return - np.log((Z1 - self.B) / (Z2 - self.B)) + (Z1 - Z2) +                self.A * np.log(((Z1 + self.B * self.mr.delta1) * (Z2 + self.B * self.mr.delta2)) /                                ((Z1 + self.B * self.mr.delta2) * (Z2 + self.B * self.mr.delta1))) /                (self.B * (self.mr.delta2 - self.mr.delta1))
