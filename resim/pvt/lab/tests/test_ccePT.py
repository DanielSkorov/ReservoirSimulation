...
class ccePT(unittest.TestCase):

  def test_01(self):
    logger.info('\nTest #01.'
                '\nComponents: N2 C1 C2 C3 C4 C5-C6 C7-C9 C10-C15 C16+')
    PP = np_array([19070., 16860., 15280., 13740., 13100., 12570., 11760.,
    ...
    res = cce(pr, PP, T, yi, flashroutine=flash.run2pPT)
    pass
  ...
  def test_02(self):
    ...
    pr = pr78(Pci, Tci, wi, mwi, dij, s0i)
    res = cce(pr, PP, T, yi, flashroutine=flash.run2pPT, repstate=BoState)
    pass
  ...
  def test_03(self):
    ...
    class RepState(BoState):
      gassep: ClassVar[Separator] = partial(ltsep, flashroutine=flash.run2pPT)
      liqsep: ClassVar[Separator] = partial(stsep, flashroutine=flash.run2pPT)
    res = cce(pr, PP, T, yi, flashroutine=flash.run2pPT, repstate=RepState)
    pass

  def test_04(self):
    logger.info('\nTest #04.\nComponents: C1 C2 C3 NC4 C5+')
    PP = np_array([26360., 23820., 21940., 19850., 17340., 16920., 16280.,
  ...
  def test_05(self):
    ...
    PP = np_array([14580.3, 12925.6, 11705.2, 10167.7, 8995.6, 8099.3,
  ...
  def test_06(self):
    logger.info('\nTest #05.\nComponents: ')
    PP = np_array([3447.4, 10342.1, 17236.9, 24131.6, 31026.4])[::-1] * 1e3
    T = 71.11 + 273.15
    ...
    res = cce(pr, PP, T, yi, flashroutine=flash.runNpPT)
    pass
